...
#define WG_MIX_FAKETCP_KPROBE_LEASE_ID_LENGTH 17U
 ...
 * making the lease mutable.
 */
static char lease_id[WG_MIX_FAKETCP_KPROBE_LEASE_ID_LENGTH + 1];
module_param_string(lease_id, lease_id, sizeof(lease_id), 0444);
MODULE_PARM_DESC(lease_id, "managed real-host legacy-kprobe generation lease");
...
static bool wg_mix_faketcp_valid_lease_id(void)
{
	...
	if (strnlen(lease_id, sizeof(lease_id)) !=
	    WG_MIX_FAKETCP_KPROBE_LEASE_ID_LENGTH || lease_id[8] != '-')
		return false;
	for (index = 0; index < WG_MIX_FAKETCP_KPROBE_LEASE_ID_LENGTH; index++) {
		if (index == 8)
			continue;
