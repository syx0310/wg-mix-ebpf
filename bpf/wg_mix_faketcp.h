...
static __always_inline int faketcp_session_snapshot_close(
	struct faketcp_session_value *session, __u64 generation,
	struct faketcp_close_snapshot *snapshot)
{
	int admitted;
...
int wg_mix_faketcp_ingress(struct xdp_md *xdp)
{
	...
	__u32 frame_len, l3_off = 0, seq, ack, next_seq;
	__u64 now;
	__u64 generation = 0;
	__s64 sum;
	...
	if (flags & (FAKETCP_FLAG_RST | FAKETCP_FLAG_FIN)) {
		...
		if (!session ||
		    !faketcp_session_snapshot_close(session, generation, &close) ||
		    close.window == 0) {
			inc_faketcp_stat(FAKETCP_STAT_BAD_STATE);
			return XDP_DROP;
		}
		if (total_len != sizeof(*iph) + sizeof(*tcp) || payload_len != 0 ||
		    ...
		    seq != close.rx_sequence || ack != close.tx_sequence ||
		    bpf_ntohs(tcp->window) != close.window || tcp->urg_ptr != 0) {
			inc_faketcp_stat(FAKETCP_STAT_BAD_PACKET);
			return XDP_DROP;
		}
		...
		if (faketcp_capture_close_packet(
			    xdp, l3.l3_off, total_len, &key, listener->wg_id,
