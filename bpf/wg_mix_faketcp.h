...
#if defined(WG_MIX_FAKETCP_PRODUCTION_BUILD) && \
	defined(WG_MIX_FAKETCP_STAGE_PROFILE)
#error "production FakeTCP objects must not enable stage profiling"
#endif

// A stage-profile object is a separately built diagnostic artifact. It keeps
// the production map/program manifest byte-for-byte compatible by temporarily
// assigning the existing 19-entry per-CPU FakeTCP stats map a profile-only
// meaning. Production objects never compile the timing helpers below.
//
// The six intervals are intentionally non-overlapping. A stage may have more
// than one source span; durations first accumulate in profile-only per-packet
// scratch. Only a fully successful direct packet publishes all six durations
// and counts together, so partial/drop paths cannot bias a report.
#ifdef WG_MIX_FAKETCP_STAGE_PROFILE
enum faketcp_stage_profile_id {
	FAKETCP_STAGE_ADMISSION_CHECKPOINT = 0,
	FAKETCP_STAGE_SESSION_LOOKUP_LOCK,
	FAKETCP_STAGE_ROOM_ADJUST,
	FAKETCP_STAGE_HEADER_MATERIALIZATION,
	FAKETCP_STAGE_PAYLOAD_CHECKSUM,
	FAKETCP_STAGE_FINAL_COMMIT,
	FAKETCP_STAGE_MAX,
};

struct faketcp_stage_profile_pending {
	__u64 duration_nanos[FAKETCP_STAGE_MAX];
	__u32 completed_mask;
	__u32 pad;
};

_Static_assert(sizeof(struct faketcp_stage_profile_pending) == 56,
	       "FakeTCP pending stage profile layout drift");

#define FAKETCP_STAGE_PROFILE_DURATION_KEY(stage) ((__u32)(stage))
#define FAKETCP_STAGE_PROFILE_COUNT_KEY(stage) \
	((__u32)FAKETCP_STAGE_MAX + (__u32)(stage))

_Static_assert(FAKETCP_STAGE_MAX * 2 <= FAKETCP_STAT_MAX,
	       "FakeTCP stage profile exceeds the existing stats map ABI");
#endif
...
// Ordinary no-XOR egress stays in one TC program. It needs only the immutable
// policy identity and stable session lifetime carried here; final mutation
// still revalidates the authority under the per-session lock. In particular,
// this projection has no nonce, token state, GSO contract, XOR state or full
// packet-geometry replay.
struct faketcp_direct_egress_admission {
	__u64 generation;
	struct faketcp_session_key key;
	struct faketcp_session_authority session_authority;
	struct faketcp_session_projection session_projection;
	__u32 rule_wg_id;
	__u32 rule_profile_id;
...
// Cross-program XOR and aggregate GSO paths retain the complete single-use
// projection. The XOR path stores it in a fresh per-CPU token slot and carries
// only its nonce through skb->cb; GSO consumes the same proof before its
// multi-segment mutation sequence.
...
struct faketcp_metadata {
	...
	__u8 direction;
	__u8 pad[3];
	// Linux 5.15 rejects XDP metadata longer than 32 bytes. Carry only the
	// stable session-lifetime authority across XDP -> TC; TC reconstructs and
	// revalidates every packet and policy field from the decoded skb and maps.
	struct faketcp_session_authority session_authority;
};
...
static __always_inline void faketcp_session_lifetime_snapshot_locked(
	const struct faketcp_session_value *session,
	struct faketcp_session_authority *authority,
	struct faketcp_session_projection *projection)
{
	authority->session_id = session->session_id;
	__builtin_memcpy(authority->runtime_incarnation,
			 session->runtime_incarnation,
			 sizeof(authority->runtime_incarnation));
	projection->local_isn = session->local_isn;
	projection->remote_isn = session->remote_isn;
	projection->window = session->window;
	projection->state = session->state;
	projection->flags = session->flags;
}
...
// XDP metadata on Linux 5.15 has only 32 bytes available: the fixed metadata
// header plus this 24-byte lifetime authority. Immutable projection fields are
// properties of the same session lifetime, so the TC consumer may reconstruct
// packet/policy state and revalidate the unique session ID + incarnation under
// the session lock without transporting a second copy of that projection.
static __always_inline int faketcp_session_compact_authority_matches_locked(
	const struct faketcp_session_value *session, __u64 generation,
	const struct faketcp_session_authority *expected_authority)
{
	__u8 difference = 0;
...
// The direct path never crosses a program boundary and never makes mutable
// revision/sequence fields admission authority. Its caller zeroes the compact
// projection once; snapshot only the lifetime fields that the final locked
// writer must compare.
static __always_inline int faketcp_session_snapshot_direct_established(
	struct faketcp_session_value *session, __u64 generation,
	struct faketcp_session_authority *authority,
	struct faketcp_session_projection *projection)
{
	...
	if (!session || !authority || !projection)
		return 0;

	bpf_spin_lock(&session->lock);
	admitted = faketcp_session_metadata_valid_locked(session, generation);
	if (admitted)
		faketcp_session_lifetime_snapshot_locked(session, authority,
							 projection);
	bpf_spin_unlock(&session->lock);
	return admitted;
}
...
static __always_inline int faketcp_session_compact_authority_matches(
	struct faketcp_session_value *session, __u64 generation,
	const struct faketcp_session_authority *expected_authority)
{
	int matched;

	if (!session || !expected_authority)
		return 0;
	bpf_spin_lock(&session->lock);
	matched = faketcp_session_compact_authority_matches_locked(
...
#ifdef WG_MIX_FAKETCP_STAGE_PROFILE
static __always_inline __u64 faketcp_stage_profile_begin(void)
{
	return bpf_ktime_get_ns();
}

// Profile artifacts dedicate faketcp_stats_map to timing data. Mixing the
// ordinary semantic counters with stage durations would make both datasets
// invalid, so those counters are deliberately disabled only in this build.
	...
	union {
		struct {
			...
			union {
				...
				// Direct admission does not use the XOR/GSO tail. A profile
				// object overlays only that existing 112-byte region with
				// per-packet pending durations, preserving the 392-byte map ABI.
				struct faketcp_stage_profile_pending stage_profile;
			};
#else
			struct faketcp_session_key key;
			struct faketcp_gso_projection gso;
...
#ifdef WG_MIX_FAKETCP_STAGE_PROFILE
static __always_inline void faketcp_stage_profile_reset(
	struct faketcp_runtime_scratch *scratch)
{
	if (scratch)
		__builtin_memset(&scratch->tc.stage_profile, 0,
				 sizeof(scratch->tc.stage_profile));
}

static __always_inline void faketcp_stage_profile_end(
	struct faketcp_runtime_scratch *scratch, __u32 stage,
	__u64 started_nanos)
{
	__u64 now;
	__u64 *pending_duration;

	if (!scratch || stage >= FAKETCP_STAGE_MAX)
		return;
	switch (stage) {
	case FAKETCP_STAGE_ADMISSION_CHECKPOINT:
		pending_duration = &scratch->tc.stage_profile.duration_nanos[0];
		break;
	case FAKETCP_STAGE_SESSION_LOOKUP_LOCK:
		pending_duration = &scratch->tc.stage_profile.duration_nanos[1];
		break;
	case FAKETCP_STAGE_ROOM_ADJUST:
		pending_duration = &scratch->tc.stage_profile.duration_nanos[2];
		break;
	case FAKETCP_STAGE_HEADER_MATERIALIZATION:
		pending_duration = &scratch->tc.stage_profile.duration_nanos[3];
		break;
	case FAKETCP_STAGE_PAYLOAD_CHECKSUM:
		pending_duration = &scratch->tc.stage_profile.duration_nanos[4];
		break;
	case FAKETCP_STAGE_FINAL_COMMIT:
		pending_duration = &scratch->tc.stage_profile.duration_nanos[5];
		break;
	default:
		return;
	}
	now = bpf_ktime_get_ns();
	if (now < started_nanos)
		return;
	*pending_duration += now - started_nanos;
	scratch->tc.stage_profile.completed_mask |= 1U << stage;
}

// Duration and count are published together only after all six direct stages
// completed. A drop leaves the pending values collection-local; the next
// egress invocation resets them before doing any work. Thus every published
// stage has exactly the same successful-packet denominator.
static __always_inline void faketcp_stage_profile_commit(
	struct faketcp_runtime_scratch *scratch)
{
	__u32 duration_key;
	__u32 count_key;
	...
	if (!scratch)
		return;
	if (scratch->tc.stage_profile.completed_mask !=
	    (1U << FAKETCP_STAGE_MAX) - 1U)
		return;
#pragma unroll
	for (int stage = 0; stage < FAKETCP_STAGE_MAX; stage++) {
		duration_key = FAKETCP_STAGE_PROFILE_DURATION_KEY(stage);
		count_key = FAKETCP_STAGE_PROFILE_COUNT_KEY(stage);
		duration = bpf_map_lookup_elem(&faketcp_stats_map,
					       &duration_key);
		count = bpf_map_lookup_elem(&faketcp_stats_map, &count_key);
		if (duration && count) {
			*duration +=
				scratch->tc.stage_profile.duration_nanos[stage];
			*count += 1;
		}
	}
}
						 ...
						 const struct faketcp_session_key *key)
{
	...
	if (copy_len == 0)
		return -1;
	copy_len -= 1;
	asm volatile("" : "+r"(copy_len));
	if (copy_len >= FAKETCP_MAX_CAPTURED_PACKET)
		return -1;
	copy_len += 1;
#else
	if (copy_len == 0 || copy_len > FAKETCP_MAX_CAPTURED_PACKET)
		return -1;
#endif
	if (bpf_skb_load_bytes(skb, info->ip_off, record->packet, copy_len) < 0)
		return -1;
	if (faketcp_output_packet_event(record, copy_len) < 0)
		return -1;
	return 0;
}
...
// Ordinary no-XOR egress does not cross a tail-call or aggregate-transform
// boundary. Validate its immutable packet/policy inputs once, snapshot only
// stable lifetime authority, and leave the 192-byte single-use token slot
// untouched. The encoder checks the compact policy identity again and the
// session writer performs the authoritative locked comparison.
static __always_inline int faketcp_direct_egress_admission_checkpoint(
	...
	__u32 mixed_wire,
	struct faketcp_runtime_scratch *scratch,
	struct faketcp_direct_egress_admission *admission)
{
	struct faketcp_tc_ipv4_udp_snapshot *headers;
	struct faketcp_session_value *session;
	struct faketcp_runtime_identity_value *identity;
	...
	__be32 remote_ipv4;
#ifdef WG_MIX_FAKETCP_STAGE_PROFILE
	__u64 admission_profile_started;
	__u64 session_profile_started;
#endif

#ifdef WG_MIX_FAKETCP_STAGE_PROFILE
	admission_profile_started = faketcp_stage_profile_begin();
#endif

	if (!scratch || !admission || !info ||
	    parser_classification != PARSE_OK || !l3 ||
	    l3->l3_off != info->ip_off || l3->l4_off != info->udp_off ||
	    l3->l4_len != sizeof(struct udphdr) + info->payload_len ||
	    info->family != FAMILY_IPV4 || skb->gso_size != 0) {
		inc_faketcp_stat(FAKETCP_STAT_BAD_PACKET);
	...
	if (!managed || !rule || !profile || generation == 0 || rule->wg_id == 0 ||
	    managed->generation != generation || rule->generation != generation ||
	    profile->generation != generation || rule->action != ACTION_REWRITE ||
	    rule->transport_mode != TRANSPORT_FAKETCP || rule->cipher_id != 0 ||
	    type_kind < 0 || type_kind >= 4 ||
	...
	if (!validate_len(type_kind, info->payload_len) ||
	...
	if (info->ip_off > skb->len || old_total_len != skb->len - info->ip_off ||
	    ...
	    faketcp_tc_key(skb, info, l3, local_ipv4, remote_ipv4, generation,
			   rule->wg_id, &admission->key) < 0) {
		inc_faketcp_stat(FAKETCP_STAT_BAD_PACKET);
		return FAKETCP_ADMISSION_DROP;
	}
#ifdef WG_MIX_FAKETCP_STAGE_PROFILE
	faketcp_stage_profile_end(scratch, FAKETCP_STAGE_ADMISSION_CHECKPOINT,
				  admission_profile_started);
	session_profile_started = faketcp_stage_profile_begin();
#endif
	session = bpf_map_lookup_elem(&faketcp_session_map, &admission->key);
	if (faketcp_session_snapshot_direct_established(
		    session, generation, &admission->session_authority,
		    &admission->session_projection)) {
#ifdef WG_MIX_FAKETCP_STAGE_PROFILE
		faketcp_stage_profile_end(scratch,
					  FAKETCP_STAGE_SESSION_LOOKUP_LOCK,
					  session_profile_started);
		admission_profile_started = faketcp_stage_profile_begin();
#endif
		identity = faketcp_runtime_identity(generation);
		if (!identity ||
...
#ifdef WG_MIX_FAKETCP_STAGE_PROFILE
		faketcp_stage_profile_end(scratch,
					  FAKETCP_STAGE_ADMISSION_CHECKPOINT,
					  admission_profile_started);
#endif
	...
	if (faketcp_capture_first_packet(skb, info, l3, rule,
					 &admission->key) < 0)
		inc_faketcp_stat(FAKETCP_STAT_EVENT_ERROR);
	return FAKETCP_ADMISSION_DROP;
}

// Cross-program XOR and aggregate GSO use this complete checkpoint. The
// shared fixed-IPv4 gate and unified skb prepare run first; prepare may
// normalize checksum/GSO metadata but never packet bytes. Policy, exact
// flow/lifetime, aggregate geometry and complete type/XOR/header composition
// are projected into one single-use token before packet-byte mutation.
...
#ifdef WG_MIX_FAKETCP_LEGACY_515
#if defined(FAKETCP_SOURCE_CONTRACT_ONLY)
/* Kept in the source contract, but never compiled into either object. */
(void)faketcp_legacy_515_validate_gso_segments(&context);
(void)faketcp_legacy_515_rewrite_gso_types(&context);
(void)faketcp_legacy_515_xor_gso_chunks(&context, xor_chunks);
#endif
...
{
#ifdef WG_MIX_FAKETCP_LEGACY_515
	(void)skb; (void)info; (void)profile; (void)cipher;
	(void)projection; (void)max_xor_target;
	return -1;
#else
	struct faketcp_gso_loop_context context = {
		.skb = skb,
...
#ifdef WG_MIX_FAKETCP_LEGACY_515
// Linux 5.15's bpf_for_each_map_elem callback ABI cannot carry a map-backed
// opaque context; fail closed for aggregate GSO rather than emit an object the
// verifier would reject or risk an unproven callback pointer.
static __always_inline int faketcp_encode_gso_segments(
	struct __sk_buff *skb, const struct packet_info *info,
	const struct faketcp_l3_info *l3,
	const struct managed_fwmark_value *managed,
	const struct egress_rule_value *rule,
	const struct profile_value *profile,
	__u64 generation, int parser_classification, int type_kind,
	__u32 standard_wire, __u32 mixed_wire)
{
	(void)skb; (void)info; (void)l3; (void)managed; (void)rule;
	(void)profile; (void)generation; (void)parser_classification;
	(void)type_kind; (void)standard_wire; (void)mixed_wire;
	inc_faketcp_stat(FAKETCP_STAT_GSO_REJECT);
	return TC_ACT_SHOT;
}
#else
			    ...
			    __u32 mixed_wire)
{
	...
	if (faketcp_egress_admission_checkpoint(
		...
		return TC_ACT_SHOT;
	if (!faketcp_egress_admission_matches(
		    skb, info, l3, managed, rule, profile, generation,
...
#pragma unroll
	...
	if (processed < payload_len) {
		...
		if (remaining > sizeof(chunk))
			return -1;
#define FAKETCP_LEGACY_LOAD_REMAINDER(length) \
		case length: \
			if (bpf_skb_load_bytes(skb, payload_off + processed, \
					       chunk, length) < 0) \
				return -1; \
			break
		switch (remaining) {
		FAKETCP_LEGACY_LOAD_REMAINDER(1);
		FAKETCP_LEGACY_LOAD_REMAINDER(2);
		FAKETCP_LEGACY_LOAD_REMAINDER(3);
		FAKETCP_LEGACY_LOAD_REMAINDER(4);
		FAKETCP_LEGACY_LOAD_REMAINDER(5);
		FAKETCP_LEGACY_LOAD_REMAINDER(6);
		FAKETCP_LEGACY_LOAD_REMAINDER(7);
		FAKETCP_LEGACY_LOAD_REMAINDER(8);
		FAKETCP_LEGACY_LOAD_REMAINDER(9);
		FAKETCP_LEGACY_LOAD_REMAINDER(10);
		FAKETCP_LEGACY_LOAD_REMAINDER(11);
		FAKETCP_LEGACY_LOAD_REMAINDER(12);
		FAKETCP_LEGACY_LOAD_REMAINDER(13);
		FAKETCP_LEGACY_LOAD_REMAINDER(14);
		FAKETCP_LEGACY_LOAD_REMAINDER(15);
		FAKETCP_LEGACY_LOAD_REMAINDER(16);
		FAKETCP_LEGACY_LOAD_REMAINDER(17);
		FAKETCP_LEGACY_LOAD_REMAINDER(18);
		FAKETCP_LEGACY_LOAD_REMAINDER(19);
		FAKETCP_LEGACY_LOAD_REMAINDER(20);
		FAKETCP_LEGACY_LOAD_REMAINDER(21);
		FAKETCP_LEGACY_LOAD_REMAINDER(22);
		FAKETCP_LEGACY_LOAD_REMAINDER(23);
		FAKETCP_LEGACY_LOAD_REMAINDER(24);
		FAKETCP_LEGACY_LOAD_REMAINDER(25);
		FAKETCP_LEGACY_LOAD_REMAINDER(26);
		FAKETCP_LEGACY_LOAD_REMAINDER(27);
		FAKETCP_LEGACY_LOAD_REMAINDER(28);
		FAKETCP_LEGACY_LOAD_REMAINDER(29);
		FAKETCP_LEGACY_LOAD_REMAINDER(30);
		FAKETCP_LEGACY_LOAD_REMAINDER(31);
		FAKETCP_LEGACY_LOAD_REMAINDER(32);
		default:
			return -1;
		}
#undef FAKETCP_LEGACY_LOAD_REMAINDER
#else
		if (bpf_skb_load_bytes(skb, payload_off + processed, chunk,
				       remaining) < 0)
			return -1;
...
static __always_inline int faketcp_direct_egress_admission_matches(
	...
	const struct profile_value *profile,
	__u64 generation,
	const struct faketcp_direct_egress_admission *admission)
{
	__u32 expected_mixed;

	if (!info || !managed || !rule || !profile || !admission ||
	    generation == 0 || admission->generation != generation ||
	    admission->key.generation != generation ||
	    managed->generation != generation || rule->generation != generation ||
	    profile->generation != generation || rule->cipher_id != 0 ||
	    rule->wg_id != admission->rule_wg_id ||
	    rule->profile_id != admission->rule_profile_id ||
	    rule->action != admission->rule_action ||
	    rule->transport_mode != admission->transport_mode ||
	    managed->action_on_miss != admission->managed_action ||
	    profile->policy_flags != admission->profile_policy_flags ||
	    admission->rule_action != ACTION_REWRITE ||
	    admission->transport_mode != TRANSPORT_FAKETCP ||
	    admission->key.wg_id != admission->rule_wg_id ||
	    admission->key.underlay_index != skb->ifindex ||
	    admission->key.local_port != info->src_port ||
	    admission->key.remote_port != info->dst_port ||
	    admission->payload_len != info->payload_len ||
	    admission->type_kind >= 4 ||
	    admission->standard_wire !=
		    wg_cpu_to_le32((__u32)admission->type_kind + 1) ||
	    profile_mixed_from_kind(profile, admission->type_kind,
				    &expected_mixed) < 0 ||
	    admission->mixed_wire != wg_cpu_to_le32(expected_mixed) ||
	    admission->session_authority.session_id == 0 ||
	    admission->session_projection.state != FAKETCP_STATE_ESTABLISHED ||
	    admission->session_projection.flags != 0 ||
	    !faketcp_runtime_incarnation_matches(
		    generation,
		    admission->session_authority.runtime_incarnation))
		return 0;
...
// The packet rewrite is shared, while each caller proves the authority shape
// appropriate to its boundary. No session field is consumed outside the final
// locked mutation in this body.
static __always_inline int faketcp_encode_established_authorized(
	struct __sk_buff *skb,
	struct packet_info *info,
	__u64 generation,
	struct faketcp_runtime_scratch *scratch,
	const struct faketcp_session_key *session_key,
	const struct faketcp_session_authority *session_authority,
	const struct faketcp_session_projection *session_projection
#ifdef WG_MIX_FAKETCP_STAGE_PROFILE
	, int stage_profile_direct
#endif
	)
{
	struct faketcp_tc_ipv4_udp_snapshot *headers;
	...
	struct faketcp_session_mutation_result mutation = {};
#ifdef WG_MIX_FAKETCP_STAGE_PROFILE
	__u64 final_profile_started;
	__u64 room_profile_started;
	__u64 header_profile_started;
	__u64 checksum_profile_started;
#endif

	if (!scratch || !info || !session_key || !session_authority ||
	    !session_projection || session_key->generation != generation) {
		inc_faketcp_stat(FAKETCP_STAT_ADMISSION_BYPASS_REJECT);
		return TC_ACT_SHOT;
	}
	headers = &scratch->tc_headers;
	...
	if (source_ipv4 != session_key->local_ipv4 ||
	    destination_ipv4 != session_key->remote_ipv4 ||
	    bpf_ntohs(old_udp.source) != session_key->local_port ||
	    bpf_ntohs(old_udp.dest) != session_key->remote_port) {
		inc_faketcp_stat(FAKETCP_STAT_ADMISSION_BYPASS_REJECT);
		return TC_ACT_SHOT;
	}
#ifdef WG_MIX_FAKETCP_STAGE_PROFILE
	if (stage_profile_direct)
		final_profile_started = faketcp_stage_profile_begin();
#endif
	session = bpf_map_lookup_elem(&faketcp_session_map, session_key);
	if (!session) {
...
#ifdef WG_MIX_FAKETCP_STAGE_PROFILE
	if (stage_profile_direct)
		faketcp_stage_profile_end(scratch, FAKETCP_STAGE_FINAL_COMMIT,
					  final_profile_started);
#endif
...
#ifdef WG_MIX_FAKETCP_STAGE_PROFILE
	if (stage_profile_direct)
		room_profile_started = faketcp_stage_profile_begin();
#endif
...
#ifdef WG_MIX_FAKETCP_STAGE_PROFILE
	if (stage_profile_direct)
		faketcp_stage_profile_end(scratch, FAKETCP_STAGE_ROOM_ADJUST,
					  room_profile_started);
#endif
	...
	if (stage_profile_direct) {
		faketcp_stage_profile_end(scratch, FAKETCP_STAGE_FINAL_COMMIT,
					  final_profile_started);
		header_profile_started = faketcp_stage_profile_begin();
	}
#endif
	...
	if (stage_profile_direct) {
		...
		checksum_profile_started = faketcp_stage_profile_begin();
	}
#endif
...
#ifdef WG_MIX_FAKETCP_STAGE_PROFILE
	if (stage_profile_direct) {
		faketcp_stage_profile_end(scratch, FAKETCP_STAGE_PAYLOAD_CHECKSUM,
					  checksum_profile_started);
		header_profile_started = faketcp_stage_profile_begin();
	}
#endif
	...
	if (stage_profile_direct) {
		faketcp_stage_profile_end(scratch,
					  FAKETCP_STAGE_HEADER_MATERIALIZATION,
					  header_profile_started);
		faketcp_stage_profile_commit(scratch);
	}
#endif
...
{
#ifdef WG_MIX_FAKETCP_STAGE_PROFILE
	__u64 admission_profile_started = faketcp_stage_profile_begin();
#endif

	if (!faketcp_direct_egress_admission_matches(
		    skb, info, managed, rule, profile, generation, admission)) {
		inc_faketcp_stat(FAKETCP_STAT_ADMISSION_BYPASS_REJECT);
		return TC_ACT_SHOT;
	}
#ifdef WG_MIX_FAKETCP_STAGE_PROFILE
	faketcp_stage_profile_end(scratch, FAKETCP_STAGE_ADMISSION_CHECKPOINT,
				  admission_profile_started);
#endif
	return faketcp_encode_established_authorized(
		skb, info, generation, scratch, &admission->key,
		&admission->session_authority, &admission->session_projection
#ifdef WG_MIX_FAKETCP_STAGE_PROFILE
		, 1
#endif
		);
...
static __always_inline int faketcp_encode_established(
	...
	const struct faketcp_egress_admission *admission)
{
	if (!rule || !admission || admission->key.generation != generation ||
	    admission->direction != FAKETCP_DIRECTION_EGRESS ||
	    admission->cipher_id != rule->cipher_id ||
	    admission->payload_len != info->payload_len ||
	...
	__u64 generation)
{
	...
	headers = &scratch->tc_headers;
	session_authority = &consumed->session_authority;
	...
	*consumed = *metadata;
	...
	if (info->ip_off > skb->len ||
	    decoded_total_len != sizeof(headers->ip) + sizeof(headers->udp) +
				 info->payload_len ||
	    bpf_ntohs(headers->udp.len) != sizeof(headers->udp) + info->payload_len ||
	    bpf_ntohs(headers->udp.source) != info->src_port ||
	    bpf_ntohs(headers->udp.dest) != info->dst_port ||
	    !faketcp_runtime_incarnation_matches(
		generation, session_authority->runtime_incarnation)) {
		inc_faketcp_stat(FAKETCP_STAT_ADMISSION_BYPASS_REJECT);
		return -1;
	}
#ifdef WG_MIX_FAKETCP_LEGACY_515
	if (decoded_total_len > 0xffff - FAKETCP_HEADER_DELTA ||
	    skb->len - info->ip_off != decoded_total_len + FAKETCP_HEADER_DELTA) {
#else
	if (skb->len - info->ip_off != decoded_total_len) {
#endif
		inc_faketcp_stat(FAKETCP_STAT_ADMISSION_BYPASS_REJECT);
		return -1;
	}
	...
	key = (struct faketcp_session_key){
		.generation = generation,
		.local_ipv4 = local_ipv4,
		.remote_ipv4 = remote_ipv4,
		.underlay_index = skb->ifindex,
		.local_port = info->dst_port,
		.remote_port = info->src_port,
		.wg_id = listener->wg_id,
	};
	if (!faketcp_session_key_valid(&key)) {
		inc_faketcp_stat(FAKETCP_STAT_ADMISSION_BYPASS_REJECT);
		return -1;
	}
	...
	session = bpf_map_lookup_elem(&faketcp_session_map, &key);
	if (!faketcp_session_compact_authority_matches(
		    session, generation, session_authority)) {
		inc_faketcp_stat(FAKETCP_STAT_ADMISSION_BYPASS_REJECT);
		return -1;
	}
	inc_faketcp_stat(FAKETCP_STAT_ADMISSION_ACCEPT);
...
#define FAKETCP_LEGACY_515_XDP_OFFSET_MASK 0x1ffffU
	...
	__u8 destination[sizeof(struct iphdr) + sizeof(struct tcphdr)])
{
	...
	case sizeof(struct ethhdr):
		source = data + sizeof(struct ethhdr);
		break;
	case sizeof(struct ethhdr) + sizeof(struct wg_vlan_hdr):
		source = data + sizeof(struct ethhdr) + sizeof(struct wg_vlan_hdr);
		break;
	case sizeof(struct ethhdr) + 2U * sizeof(struct wg_vlan_hdr):
		source = data + sizeof(struct ethhdr) +
			 2U * sizeof(struct wg_vlan_hdr);
		break;
	default:
		return -1;
	}
	...
	struct xdp_md *xdp, __u32 offset, __u32 *destination)
{
	...
	offset &= FAKETCP_LEGACY_515_XDP_OFFSET_MASK;
	source = data + offset;
...
#pragma unroll
	for (__u32 i = 0; i < sizeof(*source); i++) {
		if ((void *)(destination + i + 1) > data_end)
			return -1;
...
faketcp_xdp_ingress_body(struct xdp_md *xdp, __u64 generation)
{
	...
	*new_pseudo = (struct faketcp_pseudo_tail){
	...
	metadata->pad[1] = 0;
	metadata->pad[2] = 0;
	metadata->session_authority = admission->session_authority;
#ifdef WG_MIX_FAKETCP_LEGACY_515
...
#endif
#ifndef WG_MIX_FAKETCP_LEGACY_515
	if (bpf_xdp_adjust_tail(xdp, -FAKETCP_HEADER_DELTA) < 0) {
		inc_faketcp_stat(FAKETCP_STAT_METADATA_ERROR);
		return XDP_DROP;
	}
#else
	// Linux 5.15 generic XDP refuses bpf_xdp_adjust_tail() for near-MTU
	// skbs. The decoded IPv4 and UDP lengths already exclude the rotated
	// twelve-byte copy, so retaining it as ordinary link-layer padding is
	// equivalent to Ethernet padding; the IPv4 receive path trims it before
	// UDP/WireGuard delivery.
#endif
