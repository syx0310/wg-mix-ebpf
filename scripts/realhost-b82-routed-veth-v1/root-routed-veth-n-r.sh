...
readonly STATE_SCHEMA='owner,baseline,mutation-plan,veth,address,route,neighbor,offload,module,tested,cleanup-intent,restored'
readonly STAGE_ROOT="/run/wg-mix-ebpf-source-stages/${RUN_ID}"
readonly EXPECTED_SOURCE="${STAGE_ROOT}/source"
readonly TEMP_ROOT="${STAGE_ROOT}/go-tmp-realhost"
readonly GO_CACHE="${STAGE_ROOT}/go-cache-routed"
readonly GO_MOD_CACHE="${STAGE_ROOT}/go-mod-cache-routed"
readonly GO_PATH="${STAGE_ROOT}/go-path-routed"
...
build_argv() {
  ...
  case "${operation}" in
    ...
    temp-mkdir) OP_TARGET="${TEMP_ROOT}"; OP_ARGV=(/usr/bin/mkdir --mode=0700 -- "${TEMP_ROOT}") ;;
...
render_plan() {
  ...
  for spec in \
    'B0|evidence-mkdir' 'B1|temp-mkdir' 'B2|cache-mkdir' \
    'B3|mod-cache-mkdir' 'B4|go-path-mkdir' 'A.bpf|bpf-links' \
  ...
  for name in "${NEGATIVE_TEST}" "${POSITIVE_TESTS[@]}"; do
    plan_operation "C.${name}" "list:${name}"
  ...
  printf 'B82_ROUTED_VETH_WRITE_SET filesystem=%s,%s,%s,%s,%s network=veth:%s,%s,address:%s/%s,route:%s/%s,neighbor:%s,offload:%s:tso module=%s bpf=transient-unpinned-test-owned evidence_retained=1\n' \
    "${EVIDENCE_ROOT}" "${TEMP_ROOT}" "${GO_CACHE}" "${GO_MOD_CACHE}" "${GO_PATH}" \
...
render_mutation() {
  printf '%s\n' \
    'format=wg-mix-ebpf-b82-routed-mutation-v1' \
...
bootstrap_evidence() {
  ...
  ensure_directory temp temp-mkdir "${TEMP_ROOT}"
  ensure_directory cache cache-mkdir "${GO_CACHE}"
  ensure_directory mod-cache mod-cache-mkdir "${GO_MOD_CACHE}"
  ensure_directory go-path go-path-mkdir "${GO_PATH}"
...
ensure_baseline() {
  ...
  [[ ! -f "${MUTATION_PHASE}" ]] || fail 'mutation-without-baseline' 79
...
ensure_mutation_plan() {
  local expected
  expected="$(render_mutation)" || fail 'mutation-render'
  if [[ -f "${MUTATION_PHASE}" ]]; then
    phase_matches "${MUTATION_PHASE}" "${expected}" || fail 'mutation-drift' 79
    return
  fi
  write_phase "${MUTATION_PHASE}" "${expected}" || fail 'mutation-write' $?
}
...
read_veth_receipt() {
  local format run resource a b extra
  ...
  [[ "${format}" == 'wg-mix-ebpf-b82-routed-veth-v1' && "${run}" == "${RUN_ID}" &&
    "${resource}" == "${RESOURCE_ID}" && -z "${extra}" ]] || return 79
...
ensure_veth_phase() {
  ...
  [[ -d "/sys/class/net/${VETH_A}" && -d "/sys/class/net/${VETH_B}" ]] || fail 'partial-veth' 79
  ...
  [[ "$(/usr/bin/cat "/sys/class/net/${VETH_A}/iflink")" == "${current_b}" &&
    "$(/usr/bin/cat "/sys/class/net/${VETH_B}/iflink")" == "${current_a}" &&
    "$(/usr/bin/cat "/sys/class/net/${VETH_A}/address")" == "${VETH_A_MAC}" &&
    "$(/usr/bin/cat "/sys/class/net/${VETH_B}/address")" == "${VETH_B_MAC}" ]] || fail 'unowned-veth' 79
  if [[ "$(/usr/bin/cat "/sys/class/net/${VETH_A}/ifalias")" != "${VETH_A_ALIAS}" ]]; then run_operation N1 veth-alias-a || fail 'veth-alias-a' $?; fi
  if [[ "$(/usr/bin/cat "/sys/class/net/${VETH_B}/ifalias")" != "${VETH_B_ALIAS}" ]]; then run_operation N2 veth-alias-b || fail 'veth-alias-b' $?; fi
...
run_tests() {
  ...
  for name in "${NEGATIVE_TEST}" "${POSITIVE_TESTS[@]}"; do
    output="$(run_operation "C.${name}" "list:${name}")" || fail "test-list:${name}" $?
    [[ "$(printf '%s\n' "${output}" | /usr/bin/grep -Fxc -- "${name}")" == 1 ]] || fail "test-definition:${name}" 79
...
render_cleanup_intent() {
  printf '%s\n' \
    ...
    "veth_sha256=$(sha256_file "${VETH_PHASE}")" \
    "address_sha256=$(sha256_file "${ADDRESS_PHASE}")" \
    "route_sha256=$(sha256_file "${ROUTE_PHASE}")" \
    "neighbor_sha256=$(sha256_file "${NEIGHBOR_PHASE}")" \
    "offload_sha256=$(sha256_file "${OFFLOAD_PHASE}")" \
    "module_sha256=$(sha256_file "${MODULE_PHASE}")" \
    'reverse=module,offload,neighbor,route,address,veth'
}
...
restore_module() {
  local fields
  phase_matches "${MODULE_PHASE}" "$(render_fixed_phase module "module=${MODULE_NAME},sha256=${MODULE_SHA256},srcversion=${EXPECTED_MODULE_SRCVERSION}")" || fail 'restore-module-receipt' 79
...
restore_network() {
  if neighbor_matches; then run_operation R3 neighbor-delete || fail 'neighbor-delete' $?; fi
  [[ "$(/usr/sbin/ip -4 -j neigh show to "${REMOTE_IPV4}" dev "${VETH_A}")" == '[]' ]] || fail 'neighbor-remains' 79
  if route_matches; then run_operation R4 route-delete || fail 'route-delete' $?; fi
  [[ "$(/usr/sbin/ip -4 -j route show table main exact "${REMOTE_IPV4}/${PREFIX_BITS}")" == '[]' ]] || fail 'route-remains' 79
  if address_matches; then run_operation R5 address-delete || fail 'address-delete' $?; fi
...
validate_setup_receipts_for_restore() {
  local baseline_sha expected require_live="$1"
  ...
  if [[ "${require_live}" == 1 ]]; then
    ...
    address_matches || fail 'restore-address-live' 79
    route_matches || fail 'restore-route-live' 79
    neighbor_matches || fail 'restore-neighbor-live' 79
    offload_live_matches_desired || fail 'restore-offload-live' 79
    module_matches || fail 'restore-module-live' 79
  fi
}
...
restore_state_machine() {
  local restored phase
  bootstrap_evidence
  ensure_owner
  [[ -f "${BASELINE_PHASE}" && -f "${MUTATION_PHASE}" ]] || fail 'restore-state-incomplete' 79
  ensure_baseline
  ensure_mutation_plan
  ...
  for phase in "${VETH_PHASE}" "${ADDRESS_PHASE}" "${ROUTE_PHASE}" \
    "${NEIGHBOR_PHASE}" "${OFFLOAD_PHASE}" "${MODULE_PHASE}"; do
    [[ -f "${phase}" && ! -L "${phase}" ]] || fail 'restore-requires-reconciled-run-state' 79
  done
  if [[ -f "${CLEANUP_PHASE}" ]]; then
    validate_setup_receipts_for_restore 0
  else
    validate_setup_receipts_for_restore 1
  fi
  ensure_cleanup_intent
  assert_bpf_links_baseline
  if require_names_absent; then
    [[ ! -d "/sys/module/${MODULE_NAME}" ]] || fail 'veth-absent-module-present' 79
    restored="$(render_fixed_phase restored 'result=restored,filesystem=retained')"
  ...
  verify_veth_pair || fail 'cleanup-veth-identity' 79
  restore_module
  restore_offload
  restore_network
  assert_bpf_links_baseline
