...
func Uninstall(ctx context.Context, opts Options) (_ *Plan, retErr error) {
	...
	if err := func() (retErr error) {
		...
		if !stopped {
			...
			guardOutcome, err := guard.NewCommandExecutor(paths.VarLibDir).Cleanup(ctx)
			if err != nil {
				return fmt.Errorf("cleanup startup guard: %w", err)
			}
			if guardOutcome.Warning != nil {
				add(
					"startup guard cleanup warning after verified kernel absence: %v",
					guardOutcome.Warning,
				)
			}
		}
