...
// ValidateNoNFTOwnerState validates the durable ownership half of an explicit
// startup_guard.mode=none preflight without exposing the installation identity.
// A valid final v2 record is a stable installation identity, not evidence that
// its table is active, so callers must still require a fresh empty kernel table
// inventory before dataplane mutation. Legacy v1 records and interrupted v2
// publications remain explicit-recovery states and fail closed.
func ValidateNoNFTOwnerState(configuredStateDir string) (err error) {
	stateDir, err := openSecureStateDirectory(configuredStateDir, false)
	if errors.Is(err, os.ErrNotExist) {
		return nil
	}
	if err != nil {
	...
	if err := rejectPendingOwnerRecord(stateDir); err != nil {
		return err
	}
	if _, err := loadOwnerRecord(stateDir); err != nil && !errors.Is(err, os.ErrNotExist) {
		return fmt.Errorf("validate no-nft guard owner record: %w", err)
	}
...
func rejectPendingOwnerRecord(stateDir *secureStateDirectory) error {
	if err := stateDir.validatePath(); err != nil {
		return err
	}
	path := filepath.Join(stateDir.path, ownerRecordPendingFileName)
	file, err := guardOpenReadFileAt(stateDir.file, ownerRecordPendingFileName)
	if errors.Is(err, os.ErrNotExist) {
		return nil
	}
	if err != nil {
		return fmt.Errorf("inspect pending guard owner record %s: %w", path, err)
	}
	defer file.Close()
	info, err := file.Stat()
	if err != nil {
		return fmt.Errorf("inspect open pending guard owner record %s: %w", path, err)
	}
	if err := validateOwnerRecordFile(path, info); err != nil {
		return err
	}
	...
	if err != nil {
		return fmt.Errorf("reopen pending guard owner record %s: %w", path, err)
	}
	if !matches {
		return fmt.Errorf("pending guard owner record %s changed while opening", path)
	}
	if err := stateDir.validatePath(); err != nil {
		return err
	}
	return fmt.Errorf(
		"pending guard owner record %s may describe an interrupted v2 publication; explicit ownership recovery is required",
		path,
	)
}
