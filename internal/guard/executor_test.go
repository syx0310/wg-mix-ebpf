...
func TestDryRunExecutor(t *testing.T) {
	...
	exec := &DryRunExecutor{}
	applyOutcome, err := exec.Apply(t.Context(), plan)
	if err != nil {
		t.Fatal(err)
	}
	if applyOutcome.Observation != ObservationUnchanged || applyOutcome.Mutated || applyOutcome.Warning != nil {
		t.Fatalf("dry-run apply outcome = %#v, want unchanged", applyOutcome)
	}
	...
	cleanupOutcome, err := exec.Cleanup(t.Context())
	if err != nil {
		t.Fatal(err)
	}
	if cleanupOutcome.Observation != ObservationUnchanged || cleanupOutcome.Mutated || cleanupOutcome.Warning != nil {
		t.Fatalf("dry-run cleanup outcome = %#v, want unchanged", cleanupOutcome)
	}
	...
	observeOutcome, err := exec.Observe(t.Context())
	if err != nil || observeOutcome.Observation != ObservationUnchanged || observeOutcome.Mutated || observeOutcome.Warning != nil {
		t.Fatalf("dry-run observe outcome/error = %#v / %v, want unchanged", observeOutcome, err)
	}
}
...
func TestMissingNftBinaryCleanupFails(t *testing.T) {
	...
	if err := cleanupError(t.Context(), exec); err == nil {
...
func TestApplyReplacesExistingTableInSingleTransaction(t *testing.T) {
	...
	if err := applyError(t.Context(), exec, plan); err != nil {
		t.Fatal(err)
	}
	if len(scripts) != 1 {
...
func TestApplyDoesNotFallbackOnInvalidReplacement(t *testing.T) {
	...
	err := applyError(t.Context(), exec, plan)
...
func TestApplyRequiresReplacementPostcondition(t *testing.T) {
	...
	err := applyError(t.Context(), exec, BuildNftPlan(&control.State{}))
...
func TestCleanupWithoutOwnerRecordIsZeroWrite(t *testing.T) {
	...
	outcome, err := exec.Cleanup(t.Context())
	if err != nil {
		t.Fatal(err)
	}
	if outcome.Observation != ObservationAbsent || outcome.Mutated || outcome.Warning != nil {
		t.Fatalf("ownerless cleanup outcome = %#v, want absent without mutation", outcome)
	}
	if len(scripts) != 0 {
	...
	for _, operation := range []struct {
	...
	}{
		{
			...
			run: func(ctx context.Context, executor CommandExecutor) error {
				return applyError(ctx, executor, BuildNftPlan(&control.State{}))
			},
		},
		{
			name: "cleanup",
			run: func(ctx context.Context, executor CommandExecutor) error {
				return cleanupError(ctx, executor)
			},
		},
	} {
	...
	for _, operation := range []string{"apply", "cleanup"} {
		for _, postcondition := range []string{"unexpected-table", "inventory-error"} {
			t.Run(operation+"/"+postcondition, func(t *testing.T) {
				...
				var err error
				if operation == "apply" {
					err = applyError(t.Context(), executor, BuildNftPlan(&control.State{}))
				} else {
					err = cleanupError(t.Context(), executor)
				}
				if err == nil {
...
func TestOwnerlessCleanupRechecksEmptyPostcondition(t *testing.T) {
	...
	if outcome.Observation != ObservationUnknown || outcome.Mutated || outcome.Warning != nil {
		t.Fatalf("ownerless cleanup drift outcome = %#v, want read-only unknown", outcome)
	}
	if inventoryCalls != 2 {
...
func TestCleanupRejectsMissingIntermediateStateDirectory(t *testing.T) {
	...
	err := cleanupError(t.Context(), exec)
...
func TestCleanupRejectsForeignMarkerWithZeroWrite(t *testing.T) {
	...
	if strings.Contains(err.Error(), "foreign-owner") || strings.Contains(err.Error(), owner.Marker) {
		t.Fatalf("cleanup error exposed ownership marker material: %v", err)
	}
	if len(scripts) != 0 {
...
func TestCleanupPreservesDeleteWarningWhenExactPostconditionIsAbsent(t *testing.T) {
	stateDir := guardTestStateDir(t)
	owner := seedOwnerRecord(t, stateDir)
	inspections := 0
	mutationAttempted := false
	deleteErr := errors.New("simulated nft transaction failure")
	exec := CommandExecutor{
		StateDir: stateDir,
		listTables: func(context.Context) ([]string, error) {
	...
	outcome, err := exec.Cleanup(t.Context())
	if err != nil {
		t.Fatalf("proved-absent cleanup returned a hard error: %v", err)
	}
	if outcome.Observation != ObservationAbsent || !outcome.Mutated ||
		!errors.Is(outcome.Warning, deleteErr) {
		t.Fatalf("cleanup outcome = %#v, want absent mutation with preserved warning", outcome)
	}
}

func applyError(ctx context.Context, executor Executor, plan NftPlan) error {
	_, err := executor.Apply(ctx, plan)
	return err
}

func cleanupError(ctx context.Context, executor Executor) error {
	_, err := executor.Cleanup(ctx)
	return err
}
