...
func TestValidateLiveGuardTableShapeJSON(t *testing.T) {
	...
	cases := map[string]string{
		...
		"invalid-schema":       strings.Replace(valid, `"json_schema_version": 1`, `"json_schema_version": 0`, 1),
	}
	...
	compatible := map[string]string{
		...
		"newer-schema":        strings.Replace(valid, `"json_schema_version": 1`, `"json_schema_version": 2`, 1),
		"unknown-root-field":  strings.TrimSuffix(valid, "\n\t}") + ",\n\t\t\"future\": true\n\t}",
	}
	for name, document := range compatible {
		t.Run(name, func(t *testing.T) {
			if err := validateLiveGuardTableShapeJSON([]byte(document), owner, 41); err != nil {
				t.Fatalf("compatible live guard JSON extension was rejected: %v", err)
			}
		})
	}
