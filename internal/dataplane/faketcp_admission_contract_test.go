...
func TestFakeTCPAdmissionCheckpointDominatesEveryTransform(t *testing.T) {
	...
	if got := strings.Count(egress, "faketcp_egress_admission_checkpoint("); got != 1 {
		t.Fatalf("TC XOR checkpoint calls=%d, want exactly one", got)
	}
	if got := strings.Count(egress, "faketcp_direct_egress_admission_checkpoint("); got != 1 {
		t.Fatalf("TC direct checkpoint calls=%d, want exactly one", got)
	}
	...
	if egressParse < 0 || l3Gate < 0 || prepare < 0 || xorCheckpoint < 0 || directCheckpoint < 0 ||
		xorTypeWord < 0 || directTypeWord < 0 || xorDispatch < 0 || directEncode < 0 ||
		!(egressParse < l3Gate && l3Gate < prepare && prepare < xorCheckpoint &&
			xorCheckpoint < xorTypeWord && xorTypeWord < xorDispatch &&
			xorDispatch < directCheckpoint && directCheckpoint < directTypeWord &&
			directTypeWord < directEncode) {
		t.Fatal("fixed-IPv4/prepare gates and the boundary-specific proofs must dominate both non-GSO transforms")
	}

	checkpoints := map[string]string{
		"direct": sourceSection(t, fake,
	...
	for name, checkpointBody := range checkpoints {
		for _, mutation := range []string{
			...
			if strings.Contains(checkpointBody, mutation) {
				t.Fatalf("%s egress checkpoint mutates packet/checksum state through %q", name, mutation)
			}
		}
	}
	...
	compare := strings.Index(ingressConsume, "key = (struct faketcp_session_key)")
	...
	if !strings.Contains(fake, "_Static_assert(sizeof(struct faketcp_metadata) == 32,") ||
		strings.Contains(fake, "metadata->admission") ||
		!strings.Contains(xdp, "metadata->session_authority = admission->session_authority") ||
		!strings.Contains(ingressConsume, "faketcp_session_compact_authority_matches(") {
		t.Fatal("XDP -> TC proof must fit Linux 5.15's 32-byte metadata limit and bind stable session authority")
	}
	if !strings.Contains(xdp,
		"#ifndef WG_MIX_FAKETCP_LEGACY_515\n\tif (bpf_xdp_adjust_tail(xdp, -FAKETCP_HEADER_DELTA) < 0)") ||
		!strings.Contains(xdp, "retaining it as ordinary link-layer padding") ||
		!strings.Contains(ingressConsume,
			"skb->len - info->ip_off != decoded_total_len + FAKETCP_HEADER_DELTA") {
		t.Fatal("legacy generic-XDP ingress must avoid the near-MTU adjust-tail failure")
	}
}
...
func TestFakeTCPSingleUsePacketAdmissionProofBindsStableLifetimeAndCapabilityStaysClosed(t *testing.T) {
	...
	for _, want := range []string{
		...
		"faketcp_session_compact_authority_matches(",
...
func TestFakeTCPAdmissionStatisticsAreMutuallyExclusive(t *testing.T) {
	...
	checkpoint := sourceSection(t, text,
		"static __always_inline int faketcp_egress_admission_checkpoint(",
		"struct faketcp_gso_loop_context {")
	directCheckpoint := sourceSection(t, text,
		"static __always_inline int faketcp_direct_egress_admission_checkpoint(",
		"// Cross-program XOR and aggregate GSO use this complete checkpoint.")
	...
	if strings.Contains(checkpoint, "FAKETCP_STAT_ADMISSION_ACCEPT") ||
		strings.Contains(directCheckpoint, "FAKETCP_STAT_ADMISSION_ACCEPT") ||
