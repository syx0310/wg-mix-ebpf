...
func TestFakeTCPGSOContractIsBuildAndEvidenceGated(t *testing.T) {
	...
	if prepare < 0 || checkpoint < 0 || match < 0 || nonce < 0 || rewrite < 0 || reload < 0 ||
		sessionMutation < 0 || commitCall < 0 || consume < 0 ||
		strings.Contains(gso[:sessionMutation], "session->") ||
		strings.Count(gso, "FAKETCP_SESSION_MUTATE_TX") != 1 ||
		!(prepare < checkpoint && checkpoint < match && match < nonce && nonce < rewrite &&
			rewrite < reload && reload < sessionMutation && sessionMutation < commitCall && commitCall < consume) {
		t.Fatal("prepare/proof/segment rewrites/reload must precede one stable-lifetime writer, GSO commit, and final consume")
	}
