...
func TestFakeTCPProductionCoordinatorParserPreflightPrecedesEveryApplyMutation(t *testing.T) {
	events := &fakeTCPProductionTestEvents{}
	baseline := &fakeTCPProductionTestBaseline{events: events}
	gateCalls := 0
	planCalls := 0
	...
	shared.supervisor = lifecycle
	state := parserPreflightState(true, "transform", "auto")

	for _, operation := range []struct {
		name string
		run  func() error
	}{
		{name: "Apply", run: func() error { return coordinator.Apply(t.Context(), state) }},
		{name: "DetachStale", run: func() error {
			return coordinator.DetachStale(t.Context(), &control.State{}, state)
		}},
	} {
		err := operation.run()
		if err == nil || !strings.Contains(err.Error(), "parser \"auto\" is ambiguous") {
			t.Fatalf("%s parser preflight error = %v", operation.name, err)
		}
	}
	apply, detach, stale := baseline.counts()
	ensure, stop := lifecycle.counts()
	if gateCalls != 0 || planCalls != 0 || apply != 0 || detach != 0 || stale != 0 ||
		ensure != 0 || stop != 0 || len(events.snapshot()) != 0 {
		t.Fatalf(
			"parser preflight allowed mutation: gate=%d plan=%d baseline=%d/%d/%d lifecycle=%d/%d events=%v",
			gateCalls, planCalls, apply, detach, stale, ensure, stop, events.snapshot(),
		)
	}
	if shared.owner != dataplaneCoreOwnerUnknown || shared.scopeBound {
		t.Fatalf("parser preflight changed ownership: owner=%d bound=%t", shared.owner, shared.scopeBound)
	}
}
