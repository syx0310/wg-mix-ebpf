...
func TestFakeTCPPacketHelperSizesCannotBeZero(t *testing.T) {
	...
	for _, required := range []string{
		...
		"record->packet, copy_len",
		"faketcp_output_packet_event(record, copy_len)",
	} {
		if !strings.Contains(capture, required) {
			t.Fatalf("variable TC capture size lost verifier bound %q", required)
		}
	}
	if strings.Contains(capture, "record->packet, packet_len") ||
		strings.Contains(capture, "faketcp_output_packet_event(record, packet_len)") {
...
func TestFakeTCPTCHeaderConsumersUseHelperSnapshots(t *testing.T) {
	...
	sections := map[string]string{
		...
		"admission matcher": sourceSection(t, fake,
			...
			"// Ordinary no-XOR egress does not cross a tail-call"),
		"direct checkpoint": sourceSection(t, fake,
