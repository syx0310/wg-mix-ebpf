...
func (coordinator *fakeTCPProductionCoordinator) QuiesceForStartupGuard(
	ctx context.Context,
) (StartupGuardLease, error) {
	if coordinator == nil || coordinator.shared == nil || coordinator.shared.supervisor == nil {
		return StartupGuardLease{}, errors.New("quiesce production FakeTCP runtime for startup guard: coordinator is incomplete")
	}
	...
	if !ok {
		return StartupGuardLease{}, errors.New("quiesce production FakeTCP runtime for startup guard: supervisor has no guard barrier")
	}
	return lifecycle.QuiesceForStartupGuard(ctx)
}

func (coordinator *fakeTCPProductionCoordinator) ResumeAfterStartupGuard(
	ctx context.Context,
	...
	return lifecycle.ResumeAfterStartupGuard(ctx, lease)
}
...
func (coordinator *fakeTCPProductionCoordinator) Apply(
	...
	if err := preflightProductionUnderlayParsers(state); err != nil {
		return err
	}
...
func (coordinator *fakeTCPProductionCoordinator) DetachStale(
	...
	if err := preflightProductionUnderlayParsers(current); err != nil {
		return err
	}
