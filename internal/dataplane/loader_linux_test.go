...
func backendTestState(backend string) *control.State {
	return &control.State{
		...
		Underlays: []control.UnderlayState{{
			Name: "eth0", IfIndex: 7, Role: "underlay", Parser: "ethernet", Resolved: true,
		}},
	}
}
...
func TestResolveAttachmentBackend(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "explicit TCX", configured: exactTCXBackend, probeQuery: validQuery, query: validQuery, want: exactTCXBackend, wantProbes: 1, wantQueries: 1},
		{name: "auto prefers TCX", configured: attachmentBackendAuto, probeQuery: validQuery, query: validQuery, want: exactTCXBackend, wantProbes: 1, wantQueries: 1},
		{name: "empty is auto", configured: "", probeQuery: validQuery, query: validQuery, want: exactTCXBackend, wantProbes: 1, wantQueries: 1},
		{name: "auto falls back only on typed unsupported", configured: attachmentBackendAuto, probeErr: fmt.Errorf("probe: %w", ErrExactTCXUnsupported), want: classicTCBackend, wantProbes: 1},
		{name: "auto rejects bare probe EINVAL", configured: attachmentBackendAuto, probeErr: unix.EINVAL, wantErr: "invalid argument", wantProbes: 1},
		{name: "auto rejects bare query EINVAL", configured: attachmentBackendAuto, probeQuery: validQuery, queryErr: unix.EINVAL, wantErr: "invalid argument", wantProbes: 1, wantQueries: 1},
		{name: "auto rejects bare query unsupported errno", configured: attachmentBackendAuto, probeQuery: validQuery, queryErr: unix.EOPNOTSUPP, wantErr: "operation not supported", wantProbes: 1, wantQueries: 1},
		{name: "explicit TCX does not fall back", configured: exactTCXBackend, probeErr: fmt.Errorf("probe: %w", ErrExactTCXUnsupported), wantErr: "preflight exact TCX", wantUnsupported: true, wantProbes: 1},
		{name: "auto rejects probe permission failure", configured: attachmentBackendAuto, probeErr: unix.EPERM, wantErr: "operation not permitted", wantProbes: 1},
		{name: "auto rejects malformed probe", configured: attachmentBackendAuto, probeQuery: exactTCXQuery{}, wantErr: "no revision fence", wantProbes: 1},
		{name: "auto rejects malformed query", configured: attachmentBackendAuto, probeQuery: validQuery, query: exactTCXQuery{}, wantErr: "no revision fence", wantProbes: 1, wantQueries: 1},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			runtime := exactTCXRuntime{
				probe: func(int) (exactTCXQuery, error) {
					probes++
					return tt.probeQuery, tt.probeErr
				},
			...
			if errors.Is(err, ErrExactTCXUnsupported) != tt.wantUnsupported {
				t.Fatalf("unsupported classification = %v, want %v; error=%v", errors.Is(err, ErrExactTCXUnsupported), tt.wantUnsupported, err)
			}
			if probes != tt.wantProbes {
				t.Fatalf("TCX probes = %d, want %d", probes, tt.wantProbes)
			}
...
func TestResolveAttachmentBackendAutoRejectsNoAttachableUnderlay(t *testing.T) {
	runtime := exactTCXRuntime{
		probe: func(int) (exactTCXQuery, error) {
			t.Fatal("auto must not probe TCX without an attach target")
			return exactTCXQuery{}, nil
		},
		query: func(int, ebpf.AttachType) (exactTCXQuery, error) {
			t.Fatal("auto must not query TCX without an attach target")
			return exactTCXQuery{}, nil
		},
		attach: func(int, ebpf.AttachType, uint64, exactTCXProgram) (exactTCXKernelLink, error) {
			return nil, errors.New("unexpected attach")
...
func TestApplyRejectsAmbiguousResolvedParserBeforeDataplaneAccess(t *testing.T) {
	pinPath := filepath.Join(t.TempDir(), "must-not-be-created")
	state := backendTestState(classicTCBackend)
	state.Underlays[0].Parser = "auto"
	loader := LinuxLoader{
		PinPath:    pinPath,
		ObjectPath: filepath.Join(t.TempDir(), "must-not-be-opened.o"),
	}

	err := loader.Apply(context.Background(), state)
	if err == nil || !strings.Contains(err.Error(), "parser \"auto\" is ambiguous") {
		t.Fatalf("apply parser preflight error = %v", err)
	}
	if _, statErr := os.Lstat(pinPath); !errors.Is(statErr, os.ErrNotExist) {
		t.Fatalf("parser preflight touched pin path: %v", statErr)
	}
}
...
func TestDurableAutoTCXOwnerStillPreflightsBeforeObjectLoad(t *testing.T) {
	...
	runtime.exactTCX = exactTCXRuntime{
		probe: func(int) (exactTCXQuery, error) {
			queries++
			return exactTCXQuery{}, fmt.Errorf("durable probe: %w", ErrExactTCXUnsupported)
		},
		query: func(int, ebpf.AttachType) (exactTCXQuery, error) {
			queries++
	...
	if err == nil || !strings.Contains(err.Error(), "preflight exact TCX") ||
		!errors.Is(err, ErrExactTCXUnsupported) {
