...
func validFakeTCPAdmissionSessionStore() *fakeTCPAdmissionSessionStore {
	key := fakeTCPAdmissionSessionKey{
		...
		underlay: 2, localPort: 51820, remotePort: 443, wgID: 1,
	}
...
func TestFakeTCPAdmissionTokenIsSingleUseAndStateBound(t *testing.T) {
	...
	var gso fakeTCPTokenSlot
	gsoNonce, ok := gso.arm(projection)
	if !ok || gsoNonce == 0 || !gso.consume(gsoNonce, fakeTCPTokenArmed, projection) {
		t.Fatal("valid GSO ARMED token was rejected")
	}
	if gso.consume(gsoNonce, fakeTCPTokenArmed, projection) {
		t.Fatal("consumed GSO token replayed")
	}
	if gso.counter != gsoNonce {
