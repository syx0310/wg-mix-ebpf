...
func TestExactTCXCapabilityProbeErrorIsTheOnlyErrnoClassifier(t *testing.T) {
	for _, probeErr := range []error{
		unix.EOPNOTSUPP,
		unix.ENOSYS,
		unix.EINVAL,
	} {
		err := exactTCXCapabilityProbeError(probeErr)
		if !errors.Is(err, ErrExactTCXUnsupported) || !errors.Is(err, probeErr) {
			t.Fatalf("probe error %v classified as %v", probeErr, err)
		}
	}
	for _, probeErr := range []error{
		ciliumlink.ErrNotSupported,
		unix.EPERM,
		unix.EACCES,
		unix.ENODEV,
		errors.New("malformed query"),
	} {
		err := exactTCXCapabilityProbeError(probeErr)
		if errors.Is(err, ErrExactTCXUnsupported) || !errors.Is(err, probeErr) {
			t.Fatalf("probe error %v unexpectedly classified as %v", probeErr, err)
		}
	}
	if err := exactTCXCapabilityProbeError(nil); err != nil {
		t.Fatalf("nil probe error classified as %v", err)
	}
}
...
func (kernel *fakeExactTCXKernel) runtime() exactTCXRuntime {
	query := func(ifindex int, attach ebpf.AttachType) (exactTCXQuery, error) {
		...
		result := exactTCXQuery{Revision: kernel.revision(slot)}
		for _, state := range kernel.links {
			...
			result.Programs = append(result.Programs, exactTCXQueryProgram{
	...
	return exactTCXRuntime{
		probe: func(ifindex int) (exactTCXQuery, error) {
			return query(ifindex, ebpf.AttachTCXIngress)
		},
		query: query,
		attach: func(
			ifindex int,
