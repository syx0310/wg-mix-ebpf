...
func queryLiveExactTCX(ifindex int, attach ebpf.AttachType) (exactTCXQuery, error) {
...
func exactTCXCapabilityProbeError(err error) error {
	if err == nil {
		return nil
	}
	// QueryPrograms can synthesize link.ErrNotSupported after its own feature
	// test, including when that feature test was denied. Do not trust that broad
	// classification here: only kernel errno values returned by this fixed,
	// known-valid TCX ingress query may authorize automatic fallback.
	...
	if errors.Is(err, unix.EOPNOTSUPP) ||
		...
		return fmt.Errorf(
			"%w: fixed TCX ingress capability query failed: %w",
			ErrExactTCXUnsupported,
			err,
		)
	}
	return err
...
var liveExactTCXRuntime = exactTCXRuntime{
	probe: func(ifindex int) (exactTCXQuery, error) {
		if ifindex <= 0 {
			return exactTCXQuery{}, fmt.Errorf(
				"fixed TCX ingress capability query requires a positive ifindex, got %d",
				ifindex,
			)
		}
		query, err := queryLiveExactTCX(ifindex, ebpf.AttachTCXIngress)
		if err != nil {
			return exactTCXQuery{}, exactTCXCapabilityProbeError(err)
		}
		return query, nil
	},
	query: queryLiveExactTCX,
	attach: func(
		ifindex int,
		attach ebpf.AttachType,
		revision uint64,
