...
func (runtime *ExperimentalFakeTCPRuntime) PauseForStartupGuard(
	ctx context.Context,
) (faketcp.StartupGuardLease, error) {
	if runtime == nil || runtime.state == nil {
		return faketcp.StartupGuardLease{}, ErrExperimentalFakeTCPRuntimeClosed
	}
	state := runtime.state
	state.mu.Lock()
	if state.shutdown || state.closing || state.closed || experimentalSlowPathIsNil(state.slowPath) {
	...
	state.mu.Unlock()
	if !ok {
		return faketcp.StartupGuardLease{}, errors.New("pause FakeTCP runtime for startup guard: slow path has no pause contract")
	}
	return pauser.PauseForStartupGuard(ctx)
}

func (runtime *ExperimentalFakeTCPRuntime) ResumeAfterStartupGuard(
	ctx context.Context,
	...
	return pauser.ResumeAfterStartupGuard(ctx, lease)
}
