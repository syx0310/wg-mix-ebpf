...
func resolveAttachmentBackend(
	...
	case attachmentBackendAuto:
		...
		if errors.Is(err, ErrExactTCXUnsupported) {
			return classicTCBackend, nil
		}
		return "", err
	default:
...
func populateDataMaps(coll *ebpf.Collection, snapshot *abi.Snapshot) (returnErr error) {
	if snapshot == nil {
		return errors.New("populate static maps from nil snapshot")
	}
	control, ok := snapshot.Control[abi.ControlKeyGlobal]
	if !ok || control.ActiveGeneration == 0 {
		return errors.New("populate static maps without an active snapshot generation")
	}
	...
	defer func() {
		...
		if err := deleteGenerationMapEntries(coll, generation); err != nil {
			returnErr = errors.Join(
				returnErr,
				fmt.Errorf("rollback generation %d static map population: %w", generation, err),
			)
		}
	}()
