...
func LoadObjectTestIdentity(
	...
	if err := validateBaselineCollectionSpec(spec); err != nil {
		return ObjectIdentity{}, fmt.Errorf("validate BPF object %s: %w", identity.Source, err)
	}
...
func validatePinnedMapSpec(descriptor pinnedMapDescriptor, spec *ebpf.MapSpec) error {
	...
	if spec.NumaNode != 0 || spec.InnerMap != nil || spec.MapExtra != 0 ||
		(spec.Extra != nil && spec.Extra.Len() != 0) || len(spec.Contents) != 0 || len(spec.Tags) != 0 {
		return fmt.Errorf(
			"BPF map %q has unsupported creation metadata (numa=%d inner=%t extra=%d trailing=%t contents=%d tags=%d)",
			descriptor.name,
			spec.NumaNode, spec.InnerMap != nil, spec.MapExtra,
			spec.Extra != nil && spec.Extra.Len() != 0, len(spec.Contents), len(spec.Tags),
		)
	}
