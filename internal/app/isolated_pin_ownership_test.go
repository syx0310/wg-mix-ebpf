...
func TestIsolatedOwnershipRolesAndCleanValidationShareLease(t *testing.T) {
	...
	contractB.layout.gate = contractA.layout.gate
...
func isolatedOwnershipFixture(
	...
	leasePath := filepath.Join(lifecycleDir, "lifecycle.lease")
	gatePath := filepath.Join(lifecycleDir, "maintenance.gate")
	layout.lease = leasePath
	layout.gate = gatePath
